...
if __name__ == '__main__':
    ...
    for index, run in read_file.iterrows():
        ...
        if cat_id is None:
            ...
            no_matching_stars.append([hack_name, course_name, star_name,
                                      "Hack is missing a single star category or get_game_level_star doesn't have the name of the category in SRC"])
            continue
    ...
    for non_matching in no_matching_stars:
        print(
            f'{non_matching[0]} does not exist, has no coursed named {non_matching[1]} , or has no star named : {non_matching[2]}, my guess is: {non_matching[3]}')
